...
- Mean runtime relative error above bound: 36.68%
...
|---|---|---|---:|---|---|---:|---|---|
| run_001 | logistic_regression | accuracy around 0.93-0.95 | 0.9510 | no | under 5 | 0.50s | yes | accuracy 0.9510 outside predicted range 0.93-0.95 |
| run_002 | logistic_regression | accuracy around 0.95-0.97 | 0.9790 | no | under 5 | 0.01s | yes | accuracy 0.9790 outside predicted range 0.95-0.97 |
| run_003 | svc | accuracy around 0.96-0.98 | 0.9720 | yes | under 5 | 0.02s | yes |  |
| run_004 | gradient_boosting | accuracy around 0.96-0.98 | 0.9441 | no | under 5 | 0.45s | yes | accuracy 0.9441 outside predicted range 0.96-0.98 |
...
| run_006 | top_model_verification | accuracy around 0.97-0.99 with variance across seeds | 0.9706 | yes | under 10 | 35.67s | no | runtime 35.67s exceeded predicted 10s |
...
| run_003 | single-run best score may not justify a scientific claim; SVC with RBF kernel may overfit on small datasets without careful C tuning; feature scaling is required for SVC and is applied | none | no |
| run_004 | single-run best score may not justify a scientific claim; tree ensemble overfitting or variance on small dataset | accuracy 0.9441 outside predicted range 0.96-0.98 | no |
| run_005 | single-run best score may not justify a scientific claim; logistic regression requires feature scaling for optimal performance; small-dataset variance could overstate a model win with a single split | accuracy 0.9860 outside predicted range 0.97-0.98 | no |
| run_006 | single-run best score may not justify a scientific claim; seed variance may exceed apparent model or hyperparameter effects; Top-model gap 0.0070 is within margin 0.0100; verify before claiming a winner. | runtime 35.67s exceeded predicted 10s; Measured top-model effect is not larger than inter-seed noise. The report must not claim a robust best model. | yes |
| run_007 | small-dataset variance may overstate a model win on a single split; tree ensembles can overfit on small tabular datasets if not constrained | accuracy 0.9580 outside predicted range 0.96-0.98 | no |
